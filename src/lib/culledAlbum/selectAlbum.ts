...
/**
 * Minimum selectable (non-local) albums to warm before Select Album leaves the
 * loading state. `columns * minRows + 1` fills minRows and starts the next row
 * so content is tall enough to scroll.
 */
...
export function filterAvailableSourceAlbums(
  ...
  return siteAlbums.filter(album => !localAlbumIds.has(album.id));
}
